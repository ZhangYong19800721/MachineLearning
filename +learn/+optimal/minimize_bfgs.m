...
%minimize_bfgs BFGS拟牛顿法求解无约束最优化问题
%   参考 马昌凤“最优化计算方法及其MATLAB程序实现”（国防工业出版社） 
%   
    %% 参数设置
    if nargin <= 2 % 没有给出参数
        parameters = [];
        disp('调用minimize_bfgs函数时没有给出参数集，将使用默认参数集');
    end
    
    if ~isfield(parameters,'epsilon') % 给出参数但是没有给出epsilon
        parameters.epsilon = 1e-5; 
        disp(sprintf('调用minimize_bfgs函数时参数集中没有epsilon参数，将使用默认值%f',parameters.epsilon));
    end
    
    if ~isfield(parameters,'max_it') % 给出参数但是没有给出max_it
        parameters.max_it = 1e6;
        disp(sprintf('调用minimize_bfgs函数时参数集中没有max_it参数，将使用默认值%f',parameters.max_it));
    end
    ...
    %%
    x1 = x0;
    D = length(x0); B = eye(D); % D参数维度，B是Hessen矩阵的近似矩阵，起始时为单位阵
