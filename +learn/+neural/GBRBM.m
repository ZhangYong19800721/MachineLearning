classdef GBRBM
    ...
    methods
        ...
        function g = gradient(obj,x,i)
            ...
            v_sstd = repmat(obj.visual_sstd,1,N);
            
            %%
            h_field_0 = learn.tools.sigmoid(obj.weight_v2h * (minibatch ./ v_sstd) + h_bias);
            h_state_0 = learn.tools.sample(h_field_0);
            v_field_1 = v_sstd .* (obj.weight_v2h'* h_state_0) + v_bias;
            v_state_1 = v_field_1 + v_sstd .* randn(size(v_field_1));
            h_field_1 = learn.tools.sigmoid(obj.weight_v2h * (v_state_1 ./ v_sstd) + h_bias);
            gw = (h_field_0 * (minibatch ./ v_sstd)' - h_field_1 * (v_state_1 ./ v_sstd)') / N;
            gh = (h_field_0 - h_field_1) * ones(N,1) / N;
            gv = ((minibatch - v_state_1) ./ v_sstd.^2) * ones(N,1) / N;
            
            g1 = ((minibatch - v_bias).^2 ./ v_sstd.^3) * ones(N,1) / N;
            g2 = (minibatch ./ v_sgma.^2) .* (obj.weight_v2h' * h_field_0) * ones(N,1) / N;
            g3 = ((v_state_1 - v_bias).^2 ./ v_sgma.^3) * ones(N,1) / N;
            g4 = (v_state_1 ./ v_sgma.^2) .* (obj.weight_v2h' * h_field_1) * ones(N,1) / N;
            gd = (g1 - g2) - (g3 - g4);
        ...
        function y = object(obj,x,i)
            ...
            h_state_0 = learn.tools.sample(h_field_0);
            v_field_1 = obj.weight_v2h'* h_state_0 + v_bias;
        ...
        function v_state = likelihood_sample(obj,h_state)
            ...
            v_state = obj.likelihood(h_state);
            v_state = v_state + randn(size(v_state));
        end
        
        function v_field = likelihood(obj,h_state)
            % likelihood 计算似然概率
            % 在给定隐层神经元取值的情况下，计算显神经元的激活概率
            v_field = obj.backward(h_state);
        end
    ...
    methods(Static)
        function [] = unit_test()
            ...
            parameters.learn_rate = 1e-4;
            parameters.max_it = M*10;
            
            gbrbm = gbrbm.pretrain(data,parameters);
            
            data = reshape(data,D,[]);
            rebuild_data = gbrbm.rebuild(data);
            rebuild_data = [rebuild_data; zeros(74,S*M)];
