classdef GentleAdaBoostSSCHam
    %GentleAdaBoostSSCHam Gentle AdaBoost Similarity Sensitive Coding Hamming算法实现
    %  参考论文"Small Codes and Large Image Databases for Recognition" Section
    %  2.1, 得到的Hash码的之间的Hamming距离越大，不相似的可能性越大，Hamming距离越小
    %  相似的可能性越大
    ...
    methods(Access = private)
         %% 选择最优弱分类器
        function [t,a,b,err] = select_stump(obj,points,labels,weight,locka)
            %select_stump 在给定维度的情况下选择最优的stump参数
            % 弱分类器fm是一个stump函数，由4个参数确定：(a,b,k,t)
            % fm = a * [(x1(k) > t) == (x2(k) > t)] + b，
            % 因为要求hash码的距离为hamming距离，所以需要锁定a的值，锁定范围[0,1]
            % 输入：
    ...
    methods(Access = public)
        ...
        %% 训练
        function obj = train(obj,points,labels,locka,M)
            % train 训练BoostSSC模型
            % 输入：
            % points 数据点
    ...
    methods(Static)
        function ssc = unit_test()
            ...
            M = 10;
            ssc = ssc.train(points,labels,0.9,M);
        end
    end
end
